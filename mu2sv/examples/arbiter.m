...
ruleset u : r_t do  -- rules of units
   rule "start_requesting"
         !req[u] & !use[u] ==>  
                 begin
                 req[u] := true; 
   ...
   rule "start_using"
          grant[u] ==> 
                 begin
                 assert !Use_b "assert1"; -- exclusive access
                 /* Doesn't work, why??
                 assert forall v : 0..u-1 do !p_req[v] end; -- priority rule
                 */
                 assert req[u] "assert2"; -- reactivity
   ...
   rule "end_using"
          use[u] ==>
                 ...
                 assert !Use_b "assert3"; -- exclusive access
                 if !Req_b & !Use_b then tk[0] := true; end;
                 end;
end;

ruleset u : r_t do -- rules of arbiters
   rule "D_flip_flop"
         tk[u] ==> 
                 begin
                 flop[u] := req[u]; tk[u] := false; ntk[u] := true;
                 end;
   rule "access_granting"
         ntk[u] & flop[u] ==> 
                 begin
